-- ============================================================================
-- Migration: IMEI obligatorio para teléfonos en process_sale (Fase 1)
-- Fecha: 2026-06-02
-- Basado en: 20250131000001_fix_process_sale_stock_validation.sql
-- ============================================================================

-- Alinear constraint sale_items con IMEI 15-17 dígitos (UI POS)
...
BEGIN
    ...
    FOR item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
        ...
        -- Fase 1: teléfonos exigen IMEI válido (15-17 dígitos), una unidad por línea
        SELECT category, name, sku
        INTO v_product_category, v_product_name_db, v_product_sku_db
        FROM products
        ...
        IF v_product_category = 'phones' THEN
            v_item_imei := NULLIF(btrim(COALESCE(item->>'imei', '')), '');
            IF v_item_imei IS NULL OR NOT (v_item_imei ~ '^[0-9]{15,17}$') THEN
                RAISE EXCEPTION 'IMEI obligatorio para el teléfono % (SKU: %). Debe tener entre 15 y 17 dígitos.',
                    COALESCE(v_product_name_db, 'desconocido'), COALESCE(v_product_sku_db, 'N/A');
            END IF;
            IF v_qty <> 1 THEN
                RAISE EXCEPTION 'Los teléfonos deben registrarse una unidad por línea con su IMEI (SKU: %). Cantidad inválida: %',
                    COALESCE(v_product_sku_db, 'N/A'), v_qty;
            END IF;
        END IF;
    ...
    LOOP
        ...
        SELECT name, sku, sale_price_usd, category
        INTO v_product_name_db, v_product_sku_db, v_sale_price_usd, v_product_category
        FROM products 
        WHERE id = v_product_id AND company_id = p_company_id;
...
'Fase 1: categoría phones exige IMEI 15-17 dígitos y qty=1 por línea.
...
DO $$
BEGIN
    RAISE NOTICE '✅ Fase 1 IMEI: teléfonos requieren IMEI 15-17 dígitos en process_sale';
