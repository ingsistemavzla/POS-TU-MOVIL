-- ============================================================================
-- Migration: Corregir validación de stock para permitir ventas hasta stock cero
-- Fecha: 2025-01-25
-- Descripción: Fuerza actualización de process_sale con validación blindada
--              que permite ventas que dejen el stock en 0 (último artículo).
--              Implementa validación en 3 pasos para máxima integridad y
...
-- PASO 2: Crear la función con validación blindada en 3 pasos
...
BEGIN
    ...
    -- 6. PROCESAR ITEMS, ACTUALIZAR STOCK (LÓGICA BLINDADA EN 3 PASOS)
    FOR item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
        v_product_id := (item->>'product_id')::UUID;
        v_qty := COALESCE((item->>'qty')::NUMERIC, 1);
        ...
        WHERE
            ...
            AND store_id = p_store_id
            AND product_id = v_product_id
            AND qty >= v_qty; -- ✅ CLAVE: Mayor o IGUAL. Permite llegar a 0. Evita negativos.
