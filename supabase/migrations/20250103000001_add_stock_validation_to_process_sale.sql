-- Migration: Agregar validación de stock en process_sale para prevenir race conditions y stock negativo
-- Fecha: 2025-01-03

-- Actualizar función process_sale para validar stock antes de actualizar inventario
...
BEGIN
  ...
  -- INSERTAR ITEMS DE LA VENTA CON MANEJO ROBUSTO DE CANTIDADES Y IMEI
  -- MEJORA: VALIDAR STOCK ANTES DE ACTUALIZAR INVENTARIO
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
    ...
    -- VALIDAR STOCK ANTES DE ACTUALIZAR INVENTARIO
    ...
    -- Si no existe registro de inventario, crear uno con stock 0
    ...
    -- INSERTAR ITEM CON IMEI (NUEVO CAMPO)
    INSERT INTO sale_items (
      sale_id, product_id, qty, price_usd, subtotal_usd, product_name, product_sku, imei
    ) VALUES (
      v_sale_id, v_product_id, v_qty, v_price_usd, (v_qty * v_price_usd), v_product_name, v_product_sku,
      CASE 
        WHEN (v_item->>'imei') IS NULL OR (v_item->>'imei') = '' OR (v_item->>'imei') = 'null' THEN NULL
        ELSE v_item->>'imei'
      END
    );

    -- ACTUALIZAR INVENTARIO CON VALIDACIÓN (SOLO SI HAY SUFICIENTE STOCK)
    -- Esta validación en el WHERE previene race conditions
    UPDATE inventories 
    SET qty = qty - v_qty, 
        updated_at = now()
    WHERE product_id = v_product_id 
      AND store_id = p_store_id 
      AND company_id = p_company_id
      AND qty >= v_qty; -- ✅ VALIDACIÓN: Solo actualizar si hay suficiente stock

    -- Verificar que se actualizó una fila
    GET DIAGNOSTICS v_rows_affected = ROW_COUNT;
    
    IF v_rows_affected = 0 THEN
      -- Si no se actualizó, significa que el stock no era suficiente (race condition)
      ...
      RAISE EXCEPTION 'Stock insuficiente para el producto % (SKU: %). Stock disponible: %, solicitado: %. Posible condición de carrera.', 
...
COMMENT ON FUNCTION public.process_sale IS 'Procesa una venta con validación de stock para prevenir race conditions y stock negativo. Actualiza el inventario solo si hay suficiente stock disponible.';
