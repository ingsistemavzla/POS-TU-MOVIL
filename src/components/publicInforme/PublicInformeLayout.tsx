...
import { PUBLIC_BRAND_FOOTER, PUBLIC_INVENTORY_THEME_VARS, PUBLIC_PAGE_BG } from '@/constants/publicInventoryTheme';

function EstadoBadge({ estado }: { estado: InformeEstado }) {
  const map: Record<InformeEstado, { label: string; className: string }> = {
    aprobado: { label: 'Aprobado', className: 'bg-blue-100 text-blue-900 ring-blue-200' },
    respaldo: { label: 'Respaldo', className: 'bg-slate-100 text-slate-800 ring-slate-200' },
...
export function PublicInformeLayout({
  ...
  return (
    <div className="min-h-screen w-full text-[#0D0D0D] pb-12" style={{ ...PUBLIC_INVENTORY_THEME_VARS, backgroundColor: PUBLIC_PAGE_BG }}>
      <div className="mx-auto w-[92%] max-w-[1100px] mt-0">
        ...
        <div className="overflow-hidden rounded-xl bg-white shadow-[0_8px_24px_rgba(0,0,0,0.12)]">
          <header className="relative overflow-hidden bg-[linear-gradient(135deg,var(--verde-primario)_0%,var(--verde-oscuro)_100%)] px-5 py-12 md:px-10 md:py-16 text-center text-white">
            <div className="absolute left-4 top-4 flex items-center gap-2 rounded-full bg-white/95 px-3 py-1.5 shadow">
              <img src="/logo.svg" alt={INVENTORY_SYSTEM_NAME} className="h-6 w-6 object-contain" />
              <span className="text-xs font-bold text-[var(--verde-oscuro)]">{INVENTORY_SYSTEM_NAME}</span>
            </div>
            ...
            <p className="mt-3 text-xs text-white/70">Reference date: {informe.fecha}</p>
          </header>
          ...
          {informe.relacionados.length > 0 && (
            <footer className="px-5 py-6 md:px-10 border-t border-[#0D0D0D]/8 bg-[#fafafa]">
              <p className="text-sm font-semibold text-[var(--verde-oscuro)] mb-3">Related reports</p>
        ...
        <p className="mt-6 text-center text-xs text-[#0D0D0D]/45">{PUBLIC_BRAND_FOOTER}</p>
      </div>
    </div>
  );
}
