import {
  AlertTriangle,
  ArrowDown,
  ArrowLeftRight,
  ArrowRight,
  BarChart3,
  CheckCircle2,
  Code2,
  Database,
  FileText,
  GitBranch,
  Layers,
  Link2,
  Rocket,
  Shield,
  Target,
  Wrench,
  XCircle,
  DollarSign,
  Clock,
} from 'lucide-react';
import { INVENTORY_SYSTEM_NAME } from '@/constants/inventorySystemBranding';
import { PUBLIC_INVENTORY_THEME_VARS, PUBLIC_PAGE_BG } from '@/constants/publicInventoryTheme';
import {
  ARQUITECTURA_CONTEXTO,
  ARQUITECTURA_TECNICA_PILARES,
  BD_POS,
  BD_TECNICA,
  BENEFICIOS_ARQUITECTURA,
  CAPACIDADES_INFRAESTRUCTURA,
  CIERRE_TECNICO_INTRO,
  CIERRE_TECNICO_REQUISITOS,
  CONSIDERACIONES_FINALES,
  DEFINICION_INTRO,
  DEFINICION_PLATAFORMA,
  ENFOQUE_CORRECTO,
  ESCRITURA_HACIA_POS,
  FASES_COMERCIALES,
  INVERSION,
  LECTURA_DESDE_POS,
  MIRROR_SYNC_DESCRIPCION,
  MODULOS_SISTEMA,
  OBJETIVO_GENERAL,
  OBJETIVO_GENERAL_CIERRE,
  OBJETIVOS_FUNCIONALES,
  OBJETIVOS_SISTEMA_GARANTIAS,
  PRINCIPIO_INTEGRACION,
  PRINCIPIO_NO_ALTERACION,
  PROPUESTA_META,
  RESULTADO_ESPERADO,
  RIESGOS_INTEGRAR_EN_POS,
  SECCIONES_NAV,
} from '@/data/presupuestoServicioTecnico';

const verdeVars = PUBLIC_INVENTORY_THEME_VARS;

/** Espaciado y tipografía más compactos en móvil */
const sectionClass = 'px-4 sm:px-6 md:px-12 py-8 md:py-14 scroll-mt-4';
const cardPad = 'p-4 md:p-6';
const bodyText = 'text-xs sm:text-sm leading-snug md:leading-relaxed';
const iconSection = '[&>svg]:h-5 [&>svg]:w-5 md:[&>svg]:h-8 md:[&>svg]:w-8 shrink-0';

function SectionTitle({
  icon,
  children,
  light,
  sub,
}: {
  icon: React.ReactNode;
  children: React.ReactNode;
  light?: boolean;
  sub?: string;
}) {
  return (
    <div className="text-center mb-6 md:mb-10">
      <h2
        className={`text-lg sm:text-xl md:text-[2.2rem] font-bold pb-3 md:pb-4 relative inline-flex items-center justify-center gap-2 md:gap-3 flex-wrap ${
          light ? 'text-white' : 'text-[var(--verde-oscuro)]'
        }`}
      >
        <span className={iconSection}>{icon}</span>
        {children}
        <span
          className={`absolute bottom-0 left-1/2 -translate-x-1/2 w-[100px] h-1 rounded-sm ${
            light ? 'bg-white' : 'bg-[var(--verde-primario)]'
          }`}
        />
      </h2>
      {sub && (
        <p className={`mt-2 md:mt-4 max-w-2xl mx-auto text-xs sm:text-sm md:text-base ${light ? 'text-white/85' : 'text-[#0D0D0D]/75'}`}>
          {sub}
        </p>
      )}
    </div>
  );
}

const inversionTextDepth =
  'text-white [text-shadow:0_1px_2px_rgba(2,38,1,0.95),0_2px_6px_rgba(2,38,1,0.75),0_4px_12px_rgba(1,20,0,0.55)]';

function InversionTotalBox() {
  return (
    <div className="max-w-3xl mx-auto rounded-[10px] md:rounded-[12px] bg-[linear-gradient(135deg,var(--verde-primario),var(--verde-secundario))] p-4 md:p-6 text-center shadow-md">
      <p className={`text-xs md:text-sm font-bold uppercase ${inversionTextDepth}`}>Inversión total</p>
      <p className={`mt-1.5 md:mt-2 text-2xl sm:text-3xl md:text-4xl font-black ${inversionTextDepth}`}>
        ${INVERSION.totalUsd} USD
        <span className="text-base md:text-xl font-semibold mx-1 md:mx-2">/</span>
        {INVERSION.totalBcv} BCV
      </p>
      <p className={`mt-1.5 md:mt-2 text-xs md:text-sm ${inversionTextDepth}`}>{INVERSION.nota}</p>
    </div>
  );
}

function NavPill({ href, label }: { href: string; label: string }) {
  return (
    <a
      href={href}
      className="rounded-full border border-white/35 bg-white/15 px-3 py-1.5 text-xs font-semibold text-white hover:bg-white/25 transition-colors backdrop-blur"
    >
      {label}
    </a>
  );
}

function MirrorSyncDiagram() {
  return (
    <div className="max-w-4xl mx-auto rounded-[10px] md:rounded-[12px] border-2 border-[var(--verde-primario)]/30 bg-white p-4 md:p-8 shadow-md overflow-x-auto">
      <p className="text-center text-[10px] sm:text-xs font-bold uppercase tracking-wider text-[var(--verde-oscuro)] mb-4 md:mb-6">
        Mirror Sync Architecture — flujo controlado
      </p>
      <div className="flex flex-col md:flex-row items-stretch justify-center gap-4 md:gap-2 min-w-[280px]">
        <div className="flex-1 rounded-lg bg-[#F2F2F2] p-4 border-t-4 border-[var(--verde-oscuro)] text-center">
          <Database className="h-6 w-6 md:h-8 md:w-8 mx-auto text-[var(--verde-oscuro)] mb-1.5 md:mb-2" />
          <h4 className="font-bold text-sm text-[var(--verde-oscuro)]">{INVENTORY_SYSTEM_NAME}</h4>
          <p className="text-xs text-[#0D0D0D]/70 mt-1">Núcleo comercial · producción intacta</p>
        </div>
        <div className="flex md:flex-col items-center justify-center gap-1 text-[var(--verde-primario)] py-2">
          <div className="flex items-center gap-1 text-xs font-bold">
            <ArrowLeftRight className="h-4 w-4" />
            <span className="hidden md:inline">Sync espejo</span>
          </div>
          <div className="flex flex-col items-center text-[10px] font-semibold text-[var(--verde-oscuro)]">
            <span className="flex items-center gap-1">
              <ArrowRight className="h-3 w-3 rotate-90 md:rotate-0" /> Leer
            </span>
            <span className="flex items-center gap-1">
              <ArrowRight className="h-3 w-3 rotate-90 md:rotate-180" /> Escribir
            </span>
          </div>
        </div>
        <div className="flex-1 rounded-lg bg-blue-50 p-4 border-t-4 border-[var(--verde-primario)] text-center">
          <Wrench className="h-6 w-6 md:h-8 md:w-8 mx-auto text-[var(--verde-oscuro)] mb-1.5 md:mb-2" />
          <h4 className="font-bold text-sm text-[var(--verde-oscuro)]">Sistema técnico</h4>
          <p className="text-xs text-[#0D0D0D]/70 mt-1">BD propia · órdenes · evidencias</p>
        </div>
      </div>
      <p className="text-center text-xs text-[#0D0D0D]/65 mt-6 italic">
        Solo categoría «Servicio Técnico» — sincronización limitada y auditable
      </p>
    </div>
  );
}

function EstadosPipeline({ estados }: { estados: string[] }) {
  return (
    <div className="flex flex-wrap gap-2 mt-3">
      {estados.map((e, i) => (
        <span
          key={e}
          className="inline-flex items-center gap-1 rounded-full bg-blue-50 px-2 py-0.5 text-[10px] sm:text-xs font-medium text-[var(--verde-oscuro)]"
        >
          {i > 0 && <span className="text-[var(--verde-primario)] opacity-60">→</span>}
          {e}
        </span>
      ))}
    </div>
  );
}

export default function PresupuestoServicioTecnicoPage() {
  return (
    <div className="min-h-screen w-full text-[#0D0D0D] pb-6 md:pb-10 font-sans text-sm md:text-base" style={{ ...verdeVars, backgroundColor: PUBLIC_PAGE_BG }}>
      <div className="mx-auto w-[94%] sm:w-[90%] max-w-[1200px] mt-4 sm:mt-6 md:mt-10 overflow-hidden rounded-[10px] md:rounded-[12px] bg-white shadow-[0_8px_24px_rgba(0,0,0,0.15)]">
        {/* Header */}
        <header className="relative overflow-hidden bg-[linear-gradient(135deg,var(--verde-primario)_0%,var(--verde-oscuro)_100%)] px-4 sm:px-6 py-10 md:py-20 text-center text-white">
          <div className="absolute -top-12 -left-12 h-[400px] w-[300px] opacity-20 bg-[linear-gradient(135deg,var(--verde-secundario),var(--verde-primario))] rotate-[-15deg] rounded-[30%_70%_70%_30%/30%_30%_70%_70%]" />
          <div className="absolute -bottom-24 -right-24 h-[400px] w-[400px] opacity-20 bg-[linear-gradient(135deg,var(--verde-secundario),var(--verde-primario))] rotate-[25deg] rounded-[70%_30%_30%_70%/70%_70%_30%_30%]" />
          <div className="absolute left-3 top-3 md:left-5 md:top-5 z-[3] h-8 w-8 md:h-10 md:w-10 rounded-full bg-white shadow-md flex items-center justify-center">
            <img src="/logo.svg" alt="Logo" className="h-5 w-5 md:h-7 md:w-7 object-contain" />
          </div>
          <div className="absolute right-3 top-3 md:right-5 md:top-5 z-[3] rounded-full bg-white px-2.5 py-1 md:px-4 md:py-2 text-[10px] md:text-sm font-bold text-[var(--verde-oscuro)] shadow-md max-w-[45%] leading-tight">
            Ingenieros de sistema Venezuela
          </div>
          <div className="absolute left-3 top-12 md:left-5 md:top-[70px] z-[3] text-left text-[10px] md:text-xs text-white/90 hidden sm:block">
            <div>departamento de desarrollo</div>
            <div className="inline-flex items-center gap-1 mt-1">
              <Code2 className="h-3 w-3" />
              <span>propuesta técnica</span>
            </div>
          </div>
          <div className="relative z-[2] mx-auto max-w-[920px]">
            <div className="mx-auto mb-4 md:mb-6 flex justify-center">
              <div className="h-[72px] w-[72px] sm:h-[96px] sm:w-[96px] md:h-[120px] md:w-[120px] rounded-full border-2 md:border-4 border-white/90 bg-white/15 shadow-lg flex items-center justify-center ring-2 ring-white/40">
                <img src="/logo.svg" alt={INVENTORY_SYSTEM_NAME} className="h-12 w-12 sm:h-16 sm:w-16 md:h-20 md:w-20 object-contain" />
              </div>
            </div>
            <h1 className="font-black text-xl sm:text-2xl md:text-[3rem] leading-tight px-1">
              {PROPUESTA_META.tituloPrincipal}
            </h1>
            <p className="mt-2 md:mt-3 text-sm sm:text-base md:text-xl text-[var(--verde-secundario)] font-semibold italic">
              {PROPUESTA_META.cliente}
            </p>
            <p className="mt-3 md:mt-5 font-bold text-sm sm:text-base md:text-2xl leading-snug text-white/95 px-1">
              {PROPUESTA_META.subtitulo}
            </p>
            <p className="mt-2 md:mt-4 text-xs sm:text-sm md:text-lg opacity-90 max-w-2xl mx-auto px-1 leading-snug">
              Arquitectura híbrida paralela · módulo independiente · sincronización con {INVENTORY_SYSTEM_NAME}
            </p>
            <div className="mt-4 md:mt-6 flex flex-wrap justify-center gap-1.5 md:gap-2 max-h-[72px] md:max-h-[120px] overflow-y-auto">
              {SECCIONES_NAV.map((s) => (
                <NavPill key={s.id} href={`#${s.id}`} label={s.label} />
              ))}
            </div>
            <a
              href="#objetivo"
              className="mt-5 md:mt-8 inline-flex items-center gap-2 rounded-full bg-[var(--verde-oscuro)] px-6 py-2.5 md:px-10 md:py-4 text-sm md:text-lg font-bold text-white shadow-md hover:bg-[var(--verde-primario)] transition-colors"
            >
              <Rocket className="h-4 w-4 md:h-5 md:w-5" />
              Comenzar lectura
            </a>
          </div>
        </header>

        {/* 1 Objetivo general */}
        <section id="objetivo" className={sectionClass}>
          <SectionTitle
            icon={<Target className="text-[var(--verde-primario)]" />}
            sub="Sistema paralelo, formal y validado — sin alterar producción del inventario"
          >
            1. Objetivo general
          </SectionTitle>
          <div className="max-w-3xl mx-auto space-y-3 md:space-y-5">
            <p className={`${bodyText} text-[#0D0D0D]/90 text-center md:text-left border-l-4 border-[var(--verde-primario)] pl-3 md:pl-5`}>
              {OBJETIVO_GENERAL}
            </p>
            <p className={`${bodyText} text-[#0D0D0D]/85 text-center md:text-left bg-[#F2F2F2] rounded-lg p-3 md:p-5`}>
              {OBJETIVO_GENERAL_CIERRE}
            </p>
          </div>
        </section>

        {/* 2 Definición */}
        <section id="definicion" className={`${sectionClass} bg-[#F2F2F2] mx-2 sm:mx-4 md:mx-6 rounded-[10px] md:rounded-[12px]`}>
          <SectionTitle icon={<Layers className="text-[var(--verde-primario)]" />}>
            2. Definición general del proyecto
          </SectionTitle>
          <div className="max-w-3xl mx-auto space-y-3 md:space-y-5">
            <p className={`${bodyText} text-[#0D0D0D]/88`}>{DEFINICION_INTRO}</p>
            <p className={`${bodyText} text-[#0D0D0D]/88 font-medium`}>{DEFINICION_PLATAFORMA}</p>
            <div className={`rounded-lg bg-white ${cardPad} shadow-sm`}>
              <p className="text-xs md:text-sm font-bold text-[var(--verde-oscuro)] mb-2 md:mb-3">
                La infraestructura tecnológica especializada deberá:
              </p>
              <ul className="space-y-2">
                {CAPACIDADES_INFRAESTRUCTURA.map((c) => (
                  <li key={c} className={`flex gap-2 ${bodyText} text-[#0D0D0D]/85`}>
                    <CheckCircle2 className="h-3.5 w-3.5 md:h-4 md:w-4 shrink-0 text-[var(--verde-primario)] mt-0.5" />
                    {c}
                  </li>
                ))}
              </ul>
            </div>
          </div>
        </section>

        {/* 3 Arquitectura híbrida */}
        <section id="arquitectura" className={sectionClass}>
          <SectionTitle
            icon={<GitBranch className="text-[var(--verde-primario)]" />}
            sub="Por qué NO integrar el taller dentro del sistema comercial de inventario"
          >
            3. Arquitectura híbrida paralela
          </SectionTitle>
          <p className={`max-w-3xl mx-auto text-center ${bodyText} text-[#0D0D0D]/85 mb-4 md:mb-8`}>{ARQUITECTURA_CONTEXTO}</p>
          <div className="grid lg:grid-cols-2 gap-4 md:gap-6 max-w-5xl mx-auto">
            <div className={`rounded-[10px] md:rounded-[12px] border border-rose-200 bg-rose-50/80 ${cardPad}`}>
              <h3 className="text-sm md:text-base font-bold text-rose-950 mb-3 md:mb-4 flex items-center gap-2">
                <XCircle className="h-4 w-4 md:h-5 md:w-5 shrink-0" />
                Riesgos de incorporar el técnico dentro del sistema comercial
              </h3>
              <ul className="space-y-2">
                {RIESGOS_INTEGRAR_EN_POS.map((r) => (
                  <li key={r} className={`flex gap-2 ${bodyText} text-rose-900/90`}>
                    <span className="text-rose-500 shrink-0">✕</span>
                    {r}
                  </li>
                ))}
              </ul>
            </div>
            <div className={`rounded-[10px] md:rounded-[12px] border border-[var(--verde-primario)]/40 bg-blue-50 ${cardPad} flex flex-col justify-center`}>
              <h3 className="text-sm md:text-base font-bold text-[var(--verde-oscuro)] mb-2 md:mb-3 flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4 md:h-5 md:w-5 text-[var(--verde-primario)] shrink-0" />
                Enfoque correcto
              </h3>
              <p className={`${bodyText} text-[#0D0D0D]/88`}>{ENFOQUE_CORRECTO}</p>
            </div>
          </div>
          <div className="mt-6 md:mt-10 max-w-3xl mx-auto">
            <h3 className="text-center text-sm md:text-base font-bold text-[var(--verde-oscuro)] mb-3 md:mb-4">
              4. Objetivo general del sistema
            </h3>
            <p className={`text-center ${bodyText} text-[#0D0D0D]/75 mb-3 md:mb-4`}>
              Centralizar y controlar integralmente todos los procesos del área técnica, garantizando:
            </p>
            <div className="grid sm:grid-cols-2 gap-2 md:gap-3">
              {OBJETIVOS_SISTEMA_GARANTIAS.map((g) => (
                <div
                  key={g}
                  className={`flex gap-2 rounded-lg bg-[#F2F2F2] px-3 py-2 md:px-4 md:py-3 ${bodyText} font-medium text-[var(--verde-oscuro)]`}
                >
                  <Shield className="h-3.5 w-3.5 md:h-4 md:w-4 shrink-0 text-[var(--verde-primario)]" />
                  {g}
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* 5 Objetivos funcionales */}
        <section id="funcional" className={`${sectionClass} bg-[linear-gradient(180deg,#fafafa_0%,white_100%)]`}>
          <SectionTitle icon={<FileText className="text-[var(--verde-primario)]" />}>
            5. Objetivos funcionales específicos
          </SectionTitle>
          <div className="space-y-4 md:space-y-6 max-w-4xl mx-auto">
            {OBJETIVOS_FUNCIONALES.map((bloque, idx) => (
              <article
                key={bloque.id}
                className={`rounded-[10px] md:rounded-[12px] bg-white border border-[#e8e8e8] ${cardPad} shadow-sm`}
              >
                <h3 className="font-bold text-sm md:text-lg text-[var(--verde-oscuro)] mb-2 md:mb-3">
                  5.{idx + 1} {bloque.titulo}
                </h3>
                <ul className={`space-y-1 ${bodyText} text-[#0D0D0D]/85`}>
                  {bloque.items.map((item) => (
                    <li key={item} className="pl-2 md:pl-3 border-l-2 border-[var(--verde-secundario)]">
                      {item}
                    </li>
                  ))}
                </ul>
                {bloque.extras && (
                  <div className="mt-3 md:mt-4 pt-3 md:pt-4 border-t border-dashed border-[var(--verde-primario)]/25">
                    {bloque.id === 'ordenes' ? (
                      <>
                        <p className="text-xs font-bold uppercase text-[var(--verde-oscuro)] mb-2">
                          Estados operativos
                        </p>
                        <EstadosPipeline estados={bloque.extras} />
                      </>
                    ) : (
                      <ul className={`space-y-1.5 ${bodyText}`}>
                        {bloque.extras.map((e) => (
                          <li key={e} className="flex gap-2 text-[#0D0D0D]/85">
                            <CheckCircle2 className="h-3.5 w-3.5 md:h-4 md:w-4 shrink-0 text-[var(--verde-primario)]" />
                            {e}
                          </li>
                        ))}
                      </ul>
                    )}
                  </div>
                )}
              </article>
            ))}
          </div>
        </section>

        {/* 6 Arquitectura tecnológica */}
        <section id="tecnologia" className={sectionClass}>
          <SectionTitle icon={<Database className="text-[var(--verde-primario)]" />}>
            6. Arquitectura tecnológica propuesta
          </SectionTitle>
          <div className="grid md:grid-cols-2 gap-4 md:gap-8 max-w-4xl mx-auto">
            <div>
              <h3 className="text-sm md:text-base font-bold text-[var(--verde-oscuro)] mb-2 md:mb-3">6.1 Arquitectura híbrida paralela</h3>
              <p className={`${bodyText} text-[#0D0D0D]/80 mb-3 md:mb-4`}>
                Implementación bajo modelo desacoplado y paralelo:
              </p>
              <ul className="space-y-2">
                {ARQUITECTURA_TECNICA_PILARES.map((p) => (
                  <li key={p} className={`flex gap-2 ${bodyText}`}>
                    <span className="text-[var(--verde-primario)]">✓</span>
                    {p}
                  </li>
                ))}
              </ul>
            </div>
            <div className={`rounded-lg bg-amber-50 border border-amber-200 ${cardPad}`}>
              <h3 className="text-sm md:text-base font-bold text-amber-950 mb-2 md:mb-3 flex items-center gap-2">
                <AlertTriangle className="h-4 w-4 md:h-5 md:w-5 shrink-0" />
                6.2 Principio de no alteración del núcleo de inventario
              </h3>
              <p className="text-xs text-amber-900/80 mb-3">Bajo ningún concepto el nuevo sistema deberá:</p>
              <ul className="space-y-1.5 mb-4">
                {PRINCIPIO_NO_ALTERACION.map((p) => (
                  <li key={p} className={`${bodyText} text-amber-950 flex gap-2`}>
                    <span>•</span>
                    {p}
                  </li>
                ))}
              </ul>
              <p className={`${bodyText} font-medium text-amber-950 border-t border-amber-200 pt-2 md:pt-3`}>
                {PRINCIPIO_INTEGRACION}
              </p>
            </div>
          </div>
        </section>

        {/* 7 Integración */}
        <section id="integracion" className={`${sectionClass} bg-[#F2F2F2] mx-2 sm:mx-4 md:mx-6 rounded-[10px] md:rounded-[12px]`}>
          <SectionTitle
            icon={<Link2 className="text-[var(--verde-primario)]" />}
            sub={MIRROR_SYNC_DESCRIPCION}
          >
            7. Modelo de integración — Mirror Sync
          </SectionTitle>
          <div className="mb-6 md:mb-10">
            <MirrorSyncDiagram />
          </div>
          <div className="grid md:grid-cols-2 gap-4 md:gap-6 max-w-4xl mx-auto">
            <div className={`rounded-[10px] md:rounded-[12px] bg-white ${cardPad} shadow-sm border-t-4 border-[var(--verde-oscuro)]`}>
              <h4 className="text-sm md:text-base font-bold text-[var(--verde-oscuro)] mb-1 flex items-center gap-2">
                <ArrowRight className="h-4 w-4" />
                7.1 Leer desde {INVENTORY_SYSTEM_NAME}
              </h4>
              <p className="text-xs text-[#0D0D0D]/65 mb-3">Información estratégica</p>
              <ul className={`space-y-1 ${bodyText}`}>
                {LECTURA_DESDE_POS.map((i) => (
                  <li key={i} className="flex gap-2">
                    <span className="text-[var(--verde-primario)]">→</span>
                    {i}
                  </li>
                ))}
              </ul>
            </div>
            <div className={`rounded-[10px] md:rounded-[12px] bg-white ${cardPad} shadow-sm border-t-4 border-[var(--verde-primario)]`}>
              <h4 className="text-sm md:text-base font-bold text-[var(--verde-oscuro)] mb-1 flex items-center gap-2">
                <ArrowDown className="h-4 w-4 rotate-[-90deg]" />
                7.2 Escribir hacia {INVENTORY_SYSTEM_NAME}
              </h4>
              <p className="text-xs text-[#0D0D0D]/65 mb-3">Información operativa controlada</p>
              <ul className={`space-y-1 ${bodyText}`}>
                {ESCRITURA_HACIA_POS.map((i) => (
                  <li key={i} className="flex gap-2">
                    <span className="text-[var(--verde-primario)]">←</span>
                    {i}
                  </li>
                ))}
              </ul>
            </div>
          </div>
        </section>

        {/* 8 Bases de datos */}
        <section id="datos" className={sectionClass}>
          <SectionTitle icon={<Database className="text-[var(--verde-primario)]" />}>
            8. Bases de datos del ecosistema
          </SectionTitle>
          <div className="grid md:grid-cols-2 gap-4 md:gap-6 max-w-4xl mx-auto">
            {[BD_POS, BD_TECNICA].map((bd) => (
              <div
                key={bd.titulo}
                className={`rounded-[10px] md:rounded-[12px] ${cardPad} bg-white shadow-md border border-[#eee]`}
              >
                <h3 className="font-bold text-[var(--verde-oscuro)] mb-3 md:mb-4 text-xs md:text-base">{bd.titulo}</h3>
                <ul className="space-y-1.5 md:space-y-2">
                  {bd.items.map((item) => (
                    <li key={item} className={`${bodyText} text-[#0D0D0D]/85 flex gap-2`}>
                      <span className="w-1.5 h-1.5 rounded-full bg-[var(--verde-primario)] mt-2 shrink-0" />
                      {item}
                    </li>
                  ))}
                </ul>
              </div>
            ))}
          </div>
        </section>

        {/* 9 Módulos */}
        <section id="modulos" className={`${sectionClass} bg-[rgba(48,217,107,0.06)]`}>
          <SectionTitle icon={<Wrench className="text-[var(--verde-primario)]" />}>
            9. Módulos principales del sistema
          </SectionTitle>
          <div className="grid sm:grid-cols-2 gap-3 md:gap-4 max-w-4xl mx-auto">
            {MODULOS_SISTEMA.map((mod) => (
              <div
                key={mod.numero}
                className={`rounded-[10px] md:rounded-[12px] bg-white p-3.5 md:p-5 shadow-sm border-l-4 border-[var(--verde-primario)]`}
              >
                <span className="text-[10px] md:text-xs font-bold text-[var(--verde-primario)]">Módulo {mod.numero}</span>
                <h3 className="text-sm md:text-base font-bold text-[var(--verde-oscuro)] mt-0.5 md:mt-1">{mod.titulo}</h3>
                <p className={`${bodyText} text-[#0D0D0D]/80 mt-1.5 md:mt-2`}>{mod.descripcion}</p>
                {mod.metricas && (
                  <ul className="mt-3 pt-3 border-t border-dashed border-[#ddd] space-y-1">
                    {mod.metricas.map((m) => (
                      <li key={m} className="text-xs text-[#0D0D0D]/75">
                        · {m}
                      </li>
                    ))}
                  </ul>
                )}
              </div>
            ))}
          </div>
        </section>

        {/* Beneficios */}
        <section id="beneficios" className={sectionClass}>
          <SectionTitle icon={<BarChart3 className="text-[var(--verde-primario)]" />}>
            10. Beneficios de la arquitectura propuesta
          </SectionTitle>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-3 max-w-4xl mx-auto">
            {BENEFICIOS_ARQUITECTURA.map((b) => (
              <div
                key={b}
                className={`flex gap-2 rounded-lg bg-[#F2F2F2] px-3 py-2 md:px-4 md:py-3 ${bodyText} font-medium text-[var(--verde-oscuro)]`}
              >
                <CheckCircle2 className="h-3.5 w-3.5 md:h-4 md:w-4 shrink-0 text-[var(--verde-primario)]" />
                {b}
              </div>
            ))}
          </div>
        </section>

        {/* 11 Fases */}
        <section id="fases" className={sectionClass}>
          <SectionTitle icon={<Clock className="text-[var(--verde-primario)]" />}>
            11. Alcance del proyecto por fases
          </SectionTitle>
          <div className="space-y-4 md:space-y-6 max-w-4xl mx-auto">
            {FASES_COMERCIALES.map((fase) => (
              <article
                key={fase.numero}
                className={`rounded-[10px] md:rounded-[12px] bg-white overflow-hidden shadow-md border ${
                  fase.critica ? 'border-amber-400' : 'border-[var(--verde-primario)]/25'
                }`}
              >
                <div
                  className={`px-4 py-3 md:px-6 md:py-4 flex flex-wrap justify-between gap-2 md:gap-3 ${
                    fase.critica ? 'bg-amber-50' : 'bg-[rgba(48,217,107,0.08)]'
                  }`}
                >
                  <div className="min-w-0 flex-1">
                    <span className="text-[10px] md:text-xs font-bold uppercase text-[var(--verde-primario)]">
                      Fase {fase.numero}
                    </span>
                    <h3 className="text-base md:text-xl font-bold text-[var(--verde-oscuro)] mt-0.5 md:mt-1 leading-snug">
                      {fase.enfoque}
                    </h3>
                    <p className="text-xs md:text-sm text-gray-400 mt-0.5">{fase.titulo}</p>
                  </div>
                  <span className="inline-flex items-center gap-1 rounded-full bg-white px-3 py-1.5 md:px-4 md:py-2 text-xs md:text-sm font-bold shadow-sm shrink-0">
                    <DollarSign className="h-3.5 w-3.5 md:h-4 md:w-4 text-[var(--verde-primario)]" />
                    ${fase.valorUsd} / {fase.valorBcv} BCV
                  </span>
                </div>
                <div className="p-4 md:p-6 space-y-3 md:space-y-4">
                  {fase.critica && (
                    <p className={`${bodyText} text-amber-900 bg-amber-50 rounded-lg px-3 py-2 md:px-4 border border-amber-200`}>
                      <strong>Hito crítico:</strong> conectar ecosistemas sin afectar producción del inventario.
                    </p>
                  )}
                  <div>
                    <p className="text-xs font-bold uppercase text-[var(--verde-oscuro)] mb-1">Objetivo</p>
                    <p className={`${bodyText} text-[#0D0D0D]/85`}>{fase.objetivo}</p>
                  </div>
                  <div>
                    <p className="text-[10px] md:text-xs font-bold uppercase text-[var(--verde-oscuro)] mb-1.5 md:mb-2">Incluye</p>
                    <ul className={`space-y-0.5 md:space-y-1 ${bodyText} text-[#0D0D0D]/85`}>
                      {fase.incluye.map((i) => (
                        <li key={i} className="flex gap-2">
                          <span className="text-[var(--verde-primario)]">•</span>
                          {i}
                        </li>
                      ))}
                    </ul>
                  </div>
                  <p className={`${bodyText} font-semibold text-[var(--verde-oscuro)] flex items-center gap-2 pt-2 border-t border-[#eee]`}>
                    <CheckCircle2 className="h-3.5 w-3.5 md:h-4 md:w-4 text-[var(--verde-primario)] shrink-0" />
                    Resultado: {fase.resultado}
                  </p>
                </div>
              </article>
            ))}
          </div>

          <div className="mt-6 md:mt-10 mb-2">
            <InversionTotalBox />
          </div>
        </section>

        {/* 12 Resultado + consideraciones */}
        <section id="cierre" className={sectionClass}>
          <div id="resultado" className="scroll-mt-6">
          <SectionTitle icon={<CheckCircle2 className="text-[var(--verde-primario)]" />}>
            12. Resultado esperado
          </SectionTitle>
          <p className={`text-center max-w-2xl mx-auto ${bodyText} text-[#0D0D0D]/75 mb-4 md:mb-6`}>
            Ecosistema técnico profesional completamente independiente del sistema comercial principal, capaz de:
          </p>
          <div className="grid sm:grid-cols-2 gap-2 md:gap-3 max-w-3xl mx-auto mb-8 md:mb-14">
            {RESULTADO_ESPERADO.map((r) => (
              <div key={r} className={`flex gap-2 ${bodyText} font-medium text-[var(--verde-oscuro)] bg-[#F2F2F2] rounded-lg px-3 py-2 md:px-4 md:py-3`}>
                <CheckCircle2 className="h-3.5 w-3.5 md:h-4 md:w-4 shrink-0 text-[var(--verde-primario)]" />
                {r}
              </div>
            ))}
          </div>
          </div>

          <SectionTitle icon={<Shield className="text-[var(--verde-primario)]" />}>
            13. Consideraciones finales
          </SectionTitle>
          <ul className="max-w-2xl mx-auto space-y-3 mb-10">
            {CONSIDERACIONES_FINALES.map((c) => (
              <li key={c} className="flex gap-3 text-sm text-[#0D0D0D]/88 border-l-4 border-[var(--verde-secundario)] pl-4">
                {c}
              </li>
            ))}
          </ul>

          <div className="max-w-3xl mx-auto rounded-[10px] md:rounded-[12px] bg-[linear-gradient(135deg,var(--verde-oscuro),#0a1a08)] text-white p-5 md:p-8">
            <p className="text-center text-sm md:text-lg font-medium mb-4 md:mb-6">{CIERRE_TECNICO_INTRO}</p>
            <p className={`text-center ${bodyText} text-white/80 mb-3 md:mb-4`}>
              La implementación requiere estructura técnica rigurosa y cuidadosamente desacoplada para:
            </p>
            <div className="grid sm:grid-cols-2 gap-2">
              {CIERRE_TECNICO_REQUISITOS.map((r) => (
                <div key={r} className={`flex gap-2 ${bodyText} bg-white/10 rounded-lg px-2.5 py-1.5 md:px-3 md:py-2`}>
                  <CheckCircle2 className="h-3.5 w-3.5 md:h-4 md:w-4 shrink-0 text-[var(--verde-secundario)]" />
                  {r}
                </div>
              ))}
            </div>
          </div>
          <p className="mt-8 text-center text-xs text-[var(--verde-oscuro)]/60 font-mono">
            {PROPUESTA_META.ruta}
          </p>
        </section>

        <footer className="bg-[linear-gradient(135deg,var(--verde-primario),var(--verde-secundario))] px-4 py-8 md:px-6 md:py-12 text-center text-[var(--verde-oscuro)]">
          <p className="text-sm md:text-base font-bold">© {new Date().getFullYear()} Ingenieros de Sistema Venezuela</p>
          <p className="mt-1.5 md:mt-2 text-xs md:text-sm">
            {PROPUESTA_META.subtitulo} · {PROPUESTA_META.cliente}
          </p>
        </footer>
      </div>
    </div>
  );
}
